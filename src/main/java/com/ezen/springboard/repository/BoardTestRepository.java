...
public interface BoardTestRepository extends JpaRepository<BoardTest, Integer> {
	...
	// boardNo으로 List<BoardFileTest>를 검색하려면 findbyBoardTestBoardNo(int boardNo);
	// => SELECT * FROM T_BOARD_FILE_TEST WHERE BOARD_NO = :boardNo
	// findbyBoardTest(BoardTest boardTest);
	// AND, OR 두 컬럼 사이에 And Or를 붙여준다.
	// boardTitle, boardContent로 검색할 때 findbyBoardTitleAnd(Or)BoardContent(BoardTest boardTest);
	// => SELECT * FROM T_BOARD_TEST WHERE BOARD_TITLE = :boardTitle AND(OR) BOARD_CONTENT = :boardContent
	...
	// 	OR BOARD_WRITER LIKE '%searchKeyword%'
